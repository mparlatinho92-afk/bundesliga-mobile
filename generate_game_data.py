...
# Diese Wörter ignorieren wir in der Pyramide
BLACKLIST_LEAGUES = [
    "summe", "differenz", "gesamt", "total", "pool", "vorderpfalz-pool", "reserve", "bilanz", "auswertung"
]

# Status-Wörter (einfacher Text-Vergleich)
STATUS_KEYWORDS_SIMPLE = [
...
# Gefährliche kurze Wörter (nur als GANZES Wort suchen!)
STATUS_KEYWORDS_EXACT = ["kl", "kk", "ak"]
...
def is_status_message(text):
    """
    Prüft intelligent, ob es sich um eine Status-Nachricht handelt.
    Verhindert den 'Mecklenburg'-Fehler (kl mitten im Wort).
    """
    text_lower = text.lower()
    
    # 1. Einfache Suche für lange Wörter ("Rückzug", "Abstieg")
    for kw in STATUS_KEYWORDS_SIMPLE:
        if kw in text_lower: return True
        
    # 2. Exakte Wort-Suche für kurze Kürzel ("kl", "kk")
    # Findet "KL A" aber NICHT "Mecklenburg"
    for kw in STATUS_KEYWORDS_EXACT:
        # Regex: \b = Wortgrenze
...
def load_images_map(paths):
    ...
    for path in paths:
        if not os.path.exists(path): 
            print(f"  [Warnung] Pfad nicht gefunden: {path}")
            continue
...
def run_integrity_check(leagues, clubs, duplicates_blocked):
    report = []
    report.append("=== BUNDESLIGA ARCHITECT REPORT (V24 MECKLENBURG-FIX) ===\n")
    report.append("Grundlage: 'Vereine Übersicht Ligen Zuordnung.csv' (Spalte TM 2026)\n")
    ...
    if duplicates_blocked:
        ...
        for d in duplicates_blocked[:10]: report.append(f"  [X] {d}")
        if len(duplicates_blocked) > 10: report.append("  ... und weitere")
        report.append("")
    ...
    status_counts = {}
    for c in pool_clubs:
        raw = c['league_name_debug']
        msg = raw if is_status_message(raw) else "FEHLER: Liga nicht in Pyramide gefunden"
        status_counts[msg] = status_counts.get(msg, 0) + 1
        
    for reason, cnt in status_counts.items():
        report.append(f"  - {cnt}x Grund: '{reason}'")
        
    real_errors = [c for c in pool_clubs if not is_status_message(c['league_name_debug'])]
    if real_errors:
        report.append("\n--- [ACHTUNG] UNBEKANNTE LIGA-NAMEN ---")
        for c in real_errors:
            report.append(f"  [!] {c['name']} -> '{c['league_name_debug']}'")
...
def main():
    print("\n=== BUNDESLIGA ARCHITECT 2026 (MECKLENBURG FIX) ===")
    ...
    try:
        ...
        print(f"[FEHLER] Konnte Ligendatei nicht lesen: {e}")
        return
    ...
    try:
        ...
        print(f"[FEHLER] Konnte Vereinsdatei nicht lesen: {e}")
        return
    ...
    for c_id, data in club_db.items():
        ...
        if img:
            if not data['is_reserve'] and is_reserve_team(img['filename']): 
                pass 
            else:
                data['image_thumb'] = image_to_base64(img['full_path'])
                matched_count += 1
    
    print(f"-> {matched_count} Wappen erfolgreich zugewiesen.")
