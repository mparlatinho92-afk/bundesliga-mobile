# Wappen: was noch fehlt

Vereine mit effektiv unter 90 px ohne HD-Wappen. Gemessen mit `wappen_quality.py`
(entlarvt hochskalierte Bilder). Stand nach v0.8.100.

Gesamt: **150 Vereine**.

* 67 &ndash; keine FM-Zuordnung
* 54 &ndash; geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
* 29 &ndash; geprueft: unser Wappen ist richtig, nur grob

Quelle: sortitoutsi (nach Liga suchen) oder die Vereinsseite. Neue Datei in
`Wappen/<Verband>/<id>.png` legen, dann `python tools/wappen_intake.py --apply`
(zuschneiden, 240 px, quantisieren) und `python tools/wappen_familie.py --apply`.


## 1. Bundesliga (4)
   64 px  FC Bayern München                  geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   62 px  Hamburger SV                       geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   63 px  SSV Jahn Regensburg                geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  VfL Wolfsburg                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## 2. Bundesliga (7)
   64 px  1. FC Heidenheim 1846              geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  1. FC Kaiserslautern               geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  1. FC Magdeburg                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  1. FC Nürnberg                     geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  1. FC Union Berlin                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  Karlsruher SC                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  SV Darmstadt 98                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## 3. Liga (5)
   64 px  Holstein Kiel                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  KSV Hessen Kassel                  geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  SSVg Velbert 02                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  VSG Altglienicke                   geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  Wuppertaler SV                     geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Regionalliga Bayern (1)
   64 px  Würzburger Kickers                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Regionalliga Nord (4)
   60 px  Arminia Hannover                   geprueft: unser Wappen ist richtig, nur grob
   62 px  Hamburger SV II                    keine FM-Zuordnung
   64 px  SV Curslack-Neuengamme             geprueft: unser Wappen ist richtig, nur grob
   64 px  USC Paloma Hamburg                 keine FM-Zuordnung

## Regionalliga Nordost (2)
   62 px  FC Erzgebirge Aue                  keine FM-Zuordnung
   64 px  VFC Plauen                         geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Regionalliga Südwest (3)
   64 px  FC-Astoria Walldorf                keine FM-Zuordnung
   64 px  TSV Steinbach Haiger               geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   72 px  VfR Wormatia Worms                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Regionalliga West (3)
   64 px  1. FC Düren                        geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  Rot-Weiss Essen                    geprueft: unser Wappen ist richtig, nur grob
   64 px  SC Wiedenbrück                     keine FM-Zuordnung

## Bayernliga Nord (1)
   64 px  Würzburger FV 04                   geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Bayernliga Süd (3)
   64 px  DJK Gebenbach                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  TSV 1860 Rosenheim                 geprueft: unser Wappen ist richtig, nur grob
   64 px  TSV Schwaben Augsburg              geprueft: unser Wappen ist richtig, nur grob

## Bremen-Liga (2)
   64 px  Bremer SV                          geprueft: unser Wappen ist richtig, nur grob
   64 px  ESC Geestemünde                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Hessenliga (1)
   64 px  1. Hanauer FC 93                   keine FM-Zuordnung

## Mittelrheinliga (1)
   68 px  Bonner SC                          keine FM-Zuordnung

## NOFV-Oberliga Nord (3)
   67 px  Brandenburger SC Süd               geprueft: unser Wappen ist richtig, nur grob
   64 px  FC Anker Wismar                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  Tennis Borussia Berlin             keine FM-Zuordnung

## NOFV-Oberliga Süd (1)
   69 px  FC Viktoria 1889 Berlin            geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Oberliga Hamburg (2)
   64 px  SC Victoria Hamburg                keine FM-Zuordnung
   64 px  Wandsbeker TSV Concordia Hamburg   geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Oberliga Niederrhein (1)
   64 px  ETB Schwarz-Weiß Essen             keine FM-Zuordnung

## Oberliga Niedersachsen (3)
   64 px  Lüneburger SK Hansa                keine FM-Zuordnung
   64 px  SSV Jeddeloh II                    keine FM-Zuordnung
   64 px  SV Wilhelmshaven                   geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Oberliga Rheinland-Pfalz/Saar (2)
   64 px  FC Hertha Wiesbach                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  FC Rot-Weiß Koblenz                geprueft: unser Wappen ist richtig, nur grob

## Berlin-Liga (2)
   64 px  BFC Preussen                       geprueft: unser Wappen ist richtig, nur grob
   63 px  Steglitzer SC Südwest 1947         keine FM-Zuordnung

## Landesliga Bayern Nordost (1)
   70 px  SC 04 Schwabach                    geprueft: unser Wappen ist richtig, nur grob

## Landesliga Bayern Südost (1)
   64 px  SB Chiemgau Traunstein             keine FM-Zuordnung

## Landesliga Braunschweig (1)
   64 px  FT Braunschweig                    geprueft: unser Wappen ist richtig, nur grob

## Landesliga Bremen (2)
   62 px  ATS Buntentor Bremen               geprueft: unser Wappen ist richtig, nur grob
   64 px  TSV Osterholz-Tenever              geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Landesliga Hamburg Staffel Hammonia (3)
   62 px  Hamburger SV III                   keine FM-Zuordnung
   64 px  SSG Nikola Tesla Hamburg           keine FM-Zuordnung
   60 px  Türk-Birlikspor Pinneberg          geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Landesliga Hamburg Staffel Hansa (2)
   64 px  Hamburger Turnerschaft von 1816    keine FM-Zuordnung
   70 px  SV Altengamme                      geprueft: unser Wappen ist richtig, nur grob

## Landesliga Hannover (2)
   64 px  1. FC Wunstorf                     geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  TSV Wetschen                       geprueft: unser Wappen ist richtig, nur grob

## Landesliga Lüneburg (3)
   64 px  MTV Treubund Lüneburg              geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  TSV Etelsen                        geprueft: unser Wappen ist richtig, nur grob
   64 px  TuS Harsefeld                      geprueft: unser Wappen ist richtig, nur grob

## Landesliga Mittelrhein Staffel 1 (2)
   64 px  FV Bonn-Endenich 08                geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  FV Wiehl                           geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Landesliga Mittelrhein Staffel 2 (3)
   64 px  FC Pesch                           geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  SV Eintracht Verlautenheide        geprueft: unser Wappen ist richtig, nur grob
   64 px  Sportfreunde Düren                 geprueft: unser Wappen ist richtig, nur grob

## Landesliga Niederrhein Gruppe 2 (1)
   64 px  SV Scherpenberg                    geprueft: unser Wappen ist richtig, nur grob

## Thüringenliga (1)
   64 px  BSG Wismut Gera                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Verbandsliga Baden (2)
   64 px  FC Zuzenhausen                     geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  FC-Astoria Walldorf II             keine FM-Zuordnung

## Verbandsliga Hessen Mitte (1)
   64 px  SV Zeilsheim                       geprueft: unser Wappen ist richtig, nur grob

## Verbandsliga Hessen Nord (2)
   64 px  Lichtenauer FV                     keine FM-Zuordnung
   64 px  SSV Sand                           geprueft: unser Wappen ist richtig, nur grob

## Verbandsliga Hessen Süd (3)
   64 px  SC Dortelweil                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   62 px  SV Hummetroth                      keine FM-Zuordnung
   64 px  SV Unter-Flockenbach               geprueft: unser Wappen ist richtig, nur grob

## Verbandsliga Südbaden (1)
   64 px  Türkischer SV Singen               keine FM-Zuordnung

## Verbandsliga Südwest (3)
   64 px  SV Alemannia Waldalgesheim         geprueft: unser Wappen ist richtig, nur grob
   63 px  TB Jahn Zeiskam                    geprueft: unser Wappen ist richtig, nur grob
   64 px  VfR Kaiserslautern                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Verbandsliga Württemberg (1)
   64 px  Sportfreunde Schwäbisch Hall       geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Bezirksliga Rheinland Mitte (1)
   64 px  SV Oberzissen                      keine FM-Zuordnung

## Bezirksliga Rheinland Ost (3)
   60 px  SG Müschenbach/Hachenburg          keine FM-Zuordnung
   64 px  TuS Weitefeld-Langenbach           keine FM-Zuordnung
   64 px  TuS Westerburg                     keine FM-Zuordnung

## Bezirksliga Rheinland West (1)
   77 px  SG Wallenborn                      keine FM-Zuordnung

## Landesliga Berlin Staffel 2 (4)
   64 px  FC Internationale 1980             keine FM-Zuordnung
   64 px  FC Internationale 1980 II          keine FM-Zuordnung
   64 px  Friedenauer TSC 1886               keine FM-Zuordnung
   64 px  VSG Altglienicke II                keine FM-Zuordnung

## Landesliga Südwest Ost (7)
   62 px  FSV Schifferstadt                  geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   60 px  SV 1920 Geinsheim                  geprueft: unser Wappen ist richtig, nur grob
   62 px  SV Büchelberg                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   70 px  TSG 1846 Bretzenheim               geprueft: unser Wappen ist richtig, nur grob
   64 px  TuS Knittelsheim                   keine FM-Zuordnung
   63 px  VTG Queichhambach                  keine FM-Zuordnung
   64 px  VfB Bodenheim                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)

## Landesliga Südwest West (3)
   64 px  SV Hinterweidenthal                keine FM-Zuordnung
   64 px  SV Kirchheimbolanden               keine FM-Zuordnung
   64 px  TSC Zweibrücken                    keine FM-Zuordnung

## Verbandsliga Saarland Nord-Ost (4)
   64 px  FSG Ottweiler-Steinbach            keine FM-Zuordnung
   64 px  FSG Schiffweiler                   geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   61 px  SV Furpach                         keine FM-Zuordnung
   64 px  SV Schwarzenbach                   keine FM-Zuordnung

## Verbandsliga Saarland Süd-West (1)
   64 px  SG Perl-Besch                      keine FM-Zuordnung

## Bezirksliga Nahe (5)
   62 px  FC Brücken                         keine FM-Zuordnung
   64 px  SG Weinsheim                       keine FM-Zuordnung
   64 px  TSG Planig                         keine FM-Zuordnung
   64 px  TSV Langenlonsheim                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   61 px  TuS Hoppstädten                    keine FM-Zuordnung

## Bezirksliga Rheinhessen (8)
   62 px  FSV Nieder-Olm                     keine FM-Zuordnung
   63 px  SV 1921 Guntersblum                keine FM-Zuordnung
   61 px  SV Gimbsheim                       geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   60 px  SV Horchheim                       geprueft: unser Wappen ist richtig, nur grob
   63 px  SV Klein-Winternheim               keine FM-Zuordnung
   64 px  TuS Neuhausen                      keine FM-Zuordnung
   64 px  VfL Gundersheim                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   72 px  Wormatia Worms II                  keine FM-Zuordnung

## Bezirksliga Vorderpfalz (4)
   64 px  FG 08 Mutterstadt                  keine FM-Zuordnung
   62 px  Phönix Schifferstadt               keine FM-Zuordnung
   64 px  VfB Hassloch                       keine FM-Zuordnung
   70 px  VfR Frankenthal                    keine FM-Zuordnung

## Bezirksliga Westpfalz (5)
   64 px  FC Fehrbach                        keine FM-Zuordnung
   61 px  FC Oberarnbach                     keine FM-Zuordnung
   64 px  TSG 1904 Trippstadt                keine FM-Zuordnung
   64 px  VB Zweibrücken                     geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  VfB Reichenbach                    geprueft: unser Wappen ist richtig, nur grob

## ohne Liga (18)
   64 px  BCF Wolfratshausen                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  Concordia Wiemelhausen             geprueft: unser Wappen ist richtig, nur grob
   64 px  FC Hertha Wiesbach II              keine FM-Zuordnung
   64 px  FC International Leipzig           keine FM-Zuordnung
   69 px  FC Liria 1985 Berlin               keine FM-Zuordnung
   62 px  FC Phönix 21 Bellheim              keine FM-Zuordnung
   64 px  Fortuna Babelsberg                 geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  Oppenheim                          keine FM-Zuordnung
   64 px  SC Victoria Hamburg II             keine FM-Zuordnung
   64 px  SV Blau-Weiß St. Wendel            keine FM-Zuordnung
   64 px  SV Obersülzen                      geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   64 px  SV Viktoria Weitersburg            keine FM-Zuordnung
   64 px  SpVgg Nahbollenbach                keine FM-Zuordnung
   64 px  Sportfreunde Kladow                keine FM-Zuordnung
   64 px  TuS Holzkirchen                    geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
   63 px  TuS Niederkirchen                  keine FM-Zuordnung
   64 px  TuS Nohfelden                      keine FM-Zuordnung
   64 px  Wiedbachtaler Sportfreunde         geprueft: FM taugt nicht (fremder Verein oder anderer Ort)
